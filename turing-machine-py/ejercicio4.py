...
def generar_imagen_proceso(archivo_salida="salida.txt", nombre_archivo="proceso_mt.png"):
    ...
    try:
        ...
        # Obtener solo las cintas de los pasos (líneas con 4 partes)
        cintas = []
        ...
        # 1. Parámetros de la imagen y Colores
        PIXEL_SIZE = 2 # Tamaño del bloque de color
        
        # Colores (Paleta Esmeralda/Azul)
        COLOR_FONDO = (10, 30, 30)       # Fondo oscuro con tono azul/verde
        COLOR_BLANCO = (255, 255, 255)   # Blanco para 'B' (limpio)
        COLOR_X_ESMERALDA = (0, 150, 136) # Esmeralda/Teal para 'X' (Transformación de 0)
        COLOR_Y_AZUL_CIAN = (0, 191, 255) # Azul Cian brillante para 'Y' (Transformación de 1)
        COLOR_0_AZUL_OSCURO = (40, 80, 80) # Dark Teal para '0'
        COLOR_1_CIAN_MEDIO = (80, 160, 160) # Medium Cyan para '1'
        ...
        def get_color(symbol):
            if symbol == 'X':
                ...
                return (255, 165, 0) # Naranja para otros (por si acaso)
        ...
        for paso_idx, cinta_str in enumerate(cintas):
            for celda_idx in range(max_len):
                ...
                # Coordenadas del bloque de píxel
                x1 = celda_idx * PIXEL_SIZE
                y1 = paso_idx * PIXEL_SIZE
                x2 = x1 + PIXEL_SIZE
                y2 = y1 + PIXEL_SIZE
                
                # Dibujar el rectángulo (el píxel o bloque de píxel)
